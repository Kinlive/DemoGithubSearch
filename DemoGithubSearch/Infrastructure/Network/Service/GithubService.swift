 ...
 */
class DefaultService<TargetTypeT: TargetType>: BaseService {

  typealias TargetTypeBase = TargetTypeT
  typealias ReturnValueBase = (responseDTO: SearchResponseDTO?, error: Error?)
  ...
  private func handle(_ result: Result<Moya.Response, MoyaError>) -> ReturnValueBase {
    ...
    case .success(let response):
      do {
        ...
        return (try response.map(SearchResponseDTO.self), nil)
